...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.ndarray]:
        ...
        for sentence_pair in parallel_corpus:
            S = sentence_pair.source_tokens
            T = sentence_pair.target_tokens
            posterior = self.translation_probs[S, :][:, T]
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for sentence_pair, posterior in zip(parallel_corpus, posteriors):
            S = sentence_pair.source_tokens
            T = sentence_pair.target_tokens
            log_ = np.log(self.translation_probs[S, :][:, T] / (posterior + 1e-9) + 1e-9)
            res += (posterior * log_).sum() - T.shape[0] * np.log(S.shape[0])
        ...
        for sentence_pair, posterior in zip(parallel_corpus, posteriors):
            ...
            for s_idx, s in enumerate(S):
                for t_idx, t in enumerate(T):
                    self.translation_probs[s, t] += posterior[s_idx, t_idx]
    ...
    def align(self, sentences):
        ...
        for sentence_pair, posterior in zip(sentences, posteriors):
            alignment = []
            for tgt_index in range(len(sentence_pair.target_tokens)):
                src_index = np.argmax(posterior[:, tgt_index])
                alignment.append((src_index + 1, tgt_index + 1))
            result.append(alignment)
        return result
