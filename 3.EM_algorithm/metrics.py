...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    intersection = 0
    total_predicted = 0
    for pred, alig in zip(predicted, reference):
        possible = set(alig.possible).union(set(alig.sure))
        pred = set(pred)
        intersection += len(possible.intersection(pred))
...
def compute_recall(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for pred, alig in zip(predicted, reference):
        sure = set(alig.sure)
        pred = set(pred)
        intersection += len(sure.intersection(pred))
        total_sure += len(sure)
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    """
    pred_and_possible, pred = compute_precision(reference, predicted)
    pred_and_sure, sure = compute_recall(reference, predicted)
    return 1 - (pred_and_possible + pred_and_sure) / (pred + sure)
